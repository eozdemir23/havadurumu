...
if (fs.existsSync(DATA_FILE)) {
    try { ayarlar = JSON.parse(fs.readFileSync(DATA_FILE)); } catch (e) { ayarlar = { favoriler: [], sonDurumlar: {}, dil: 'tr' }; }
}
...
const getAQIInfo = (aqi) => {
    ...
    return colors[aqi - 1] ? colors[aqi - 1](list[aqi - 1]) : chalk.gray("Bilinmiyor");
};
...
const grafikCiz = (tahminler) => {
    if (!tahminler || tahminler.length === 0) return;
    ...
    tahminler.forEach(t => {
        ...
        const barLen = Math.round(((t.main.temp - min) / (max - min || 1)) * 12) + 2;
        const bar = "█".repeat(barLen > 0 ? barLen : 1);
...
const yardimGoster = () => {
    ...
    console.log(chalk.yellow("5. liste") + "        : Favorileri gösterir");
    console.log(chalk.yellow("6. exit") + "         : Uygulamadan çıkar");
    console.log(chalk.blue("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"));
...
const havaDurumuSorgula = async (sehir, sessiz = false) => {
    try {
        ...
        const { coord, name, main, weather, visibility } = res.data;
        ...
        console.log(chalk.red("\n❌ Hata: Şehir bulunamadı veya API hatası."));
        anaMenu();
    }
};
...
// --- BAŞLATMA VE NEVRUZ KONTROLÜ ---
console.clear();
const bugun = new Date();
if (bugun.getMonth() === 2) { // Mart ayı
    console.log(chalk.bold.white.bgGreen(" 🌸 HOŞ GELDİN MART! 21 MART NEVRUZ BAYRAMI YAKLAŞIYOR 🔥 "));
}
console.log(chalk.bold.white.bgMagenta("      WEATHER ULTIMATE v14.0 (HER ŞEY DAHİL)      "));

// IP API - HTTPS destekli yeni adres
axios.get('https://ipapi.co/json/').then(res => {
    if (res.data && res.data.city) {
        console.log(chalk.dim(`📍 Otomatik konum: ${res.data.city}`));
        havaDurumuSorgula(res.data.city, true);
    } else {
        anaMenu();
    }
}).catch(() => {
    console.log(chalk.dim("📍 Otomatik konum belirlenemedi."));
    anaMenu();
});